...
% % ============================================================== 
 ...
 for sl=3:3
 cd(dr1);
 load(strcat('Spiral_cine_3T_1leave_FB_014_slice',num2str(sl),'_full.mat'),'kdata','ktraj','dcf');
 cd(dr2);
...
% ================================================================
kdata=reshape(kdata,[nFreqEncoding*ninterleavesPerFrame*numFramesToKeep,nChannelsToChoose]);
[vkdata,vcoilImages] = combine_coils_covar(kdata,coilImages1,coilImages2,0.80);
...
N1 = 400;%ceil(max([max(abs(real(ktraj_com(:)))),max(abs(imag(ktraj_com(:))))]));
...
load('tmp_L.mat');
tic;lowResRecons = l2Recont_v2(kdata_com,FT_LR,csm_lowRes,0.01,N1,L);toc
...
[~,~,L]=estimateLapKernelLR(X1,sigma,0.001);
%% ==============================================================
