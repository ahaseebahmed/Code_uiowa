...
% ================================================================
kdata=reshape(kdata,[nFreqEncoding*ninterleavesPerFrame*numFramesToKeep,nChannelsToChoose]);
[vkdata,vcoilImages] = combine_coils_covar(kdata,coilImages1,coilImages2,0.9997);
...
tic;lowResRecons = l2Recont_encoder(kdata,FT_LR,csm,0.01,N,W1,W2,b1,b2);toc
...
%   q= fft(X,[],2);
%  q(:,numFramesToKeep/2-numFramesToKeep/4-75:numFramesToKeep/2+numFramesToKeep/4+75)=0;
%  X1 = ifft(q,[],2);
...
% cd(dr1);
% save(strcat('resnn_15_',num2str(sl),'.mat'), 'x','V','-v7.3');
% cd(dr2);
 end
