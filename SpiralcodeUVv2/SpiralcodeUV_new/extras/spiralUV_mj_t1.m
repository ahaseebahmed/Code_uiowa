...
% addpath(genpath('/Users/ahhmed/Downloads/gpuNUFFT-master/gpuNUFFT'));
...
str=num2str(pt_no(p));
 load(strcat('/Data/Spiral_cine_3T_1leave_FB_00',str,'_full.mat'),'kdata','ktraj','dcf');
 load(strcat('/Data/cRatioI_s00',str,'.mat'));
...
tic;lowResRecons = l2Recont1(kdata_com,ktraj_com,coilImages,0.01);toc
[N1,~,~] = size(lowResRecons);
[~,Si,L]=estimateLapKernelLR(reshape(lowResRecons,[N1*N1,numFramesToKeep]),2.5,0.001);
[~,Sbasis,V]=svd(L);
...
%save('res_wo_storm.mat','y');
save(strcat('result_new',str,'.mat'), 'y','lowResRecons','-v7.3');
end