...
for p=1:size(pt_no,2)
 ...
 str=num2str(pt_no(p));
 load(strcat('/Data/Spiral_cine_3T_1leave_FB_0',str,'_VD1_full.mat'),'kdata','ktraj','dcf');
...
AtA = @(x) AtA_UV(FT,x,V,csm_lowRes,size(kdata_com,1)) + 0.1*Reg(x);
tic; x1 = pcg(AtA,Atb(:),1e-5,50);toc;
lowResRecons = reshape(reshape(x1,[N1*N1,nBasis])*V',[N1,N1,numFramesToKeep]);
[~,Si,L]=estimateLapKernelLR(reshape(lowResRecons,[N1*N1,numFramesToKeep]),2.5,2);
[~,Sbasis,V]=svd(L);
V=V(:,end-nBasis+1:end);
Sbasis=Sbasis(end-nBasis+1:end,end-nBasis+1:end);
end
...
save(strcat('res_new_iter1',str,'.mat'), 'y','lowResRecons','-v7.3');
end
