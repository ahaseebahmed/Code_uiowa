...
pt_no=[27,7,9,10,11,28];
sigma=[4.5];
lam=[0.3];
sigma2=0.8;%[0.2,0.3,0.4];
 %strr={'UD','VD'};
...
N1 = 28;%ceil(max([max(abs(real(ktraj_com(:)))),max(abs(imag(ktraj_com(:))))]));
...
%%
%clear kdata csm V ;
mkdir './../../Data/Results/res_reducedLR';
cd './../../Data/Results/res_reducedLR';
