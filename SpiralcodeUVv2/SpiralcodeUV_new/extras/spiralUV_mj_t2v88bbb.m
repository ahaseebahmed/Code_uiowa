...
AtA = @(x) AtA_UV(FT,x,V,csm_lowRes,size(kdata_com,1)) + (sigma2(ll)/ii)*Reg(x);
tic; x1 = pcg(AtA,Atb(:),1e-5,50);toc;
lowResRecons = reshape(reshape(x1,[N1*N1,nBasis])*V',[N1,N1,numFramesToKeep]);
[~,tmp,L]=estimateLapKernelLR(reshape(lowResRecons,[N1*N1,numFramesToKeep]),sigma(kk),lam(jj));
[~,Sbasis,V]=svd(L);
V=V(:,end-nBasis+1:end);
Sbasis=Sbasis(end-nBasis+1:end,end-nBasis+1:end);
...
%%
%clear kdata csm V ;
mkdir './../../Data/Results/res_23May';
cd './../../Data/Results/res_23May';
save(strcat('res000_',str,num2str(sigma2(ll)),'_',num2str(sigma(kk)),'_',num2str(lam(jj)),'.mat'), 'y','lowResRecons','-v7.3');
