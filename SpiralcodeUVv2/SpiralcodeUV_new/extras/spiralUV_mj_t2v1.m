...
addpath('./../../Data/SpiralData_Virg/Perfusion/Perfusion');
addpath('./csm');
...
% % ============================================================== 
%Load data
  load('./../../Data/SpiralData_Virg/Perfusion/Perfusion/Human_PT_007_stress_slice3_organized.mat','kdata','ktrajm','dcf');
...
for p=1:size(pt_no)
 ...
 str=num2str(pt_no(p));
 load(strcat('/Data/Spiral_cine_3T_1leave_FB_0',str,'_full.mat'),'kdata','ktrajm','dcf');
...
%save('LRPt27_11.mat','lowResRecons');
...
for ii=1:1
%[~,Si,L]=estimateLapKernelLRv1(reshape(lowResRecons,[N1*N1,numFramesToKeep]),reshape(lowimg,[N1*N1,numFramesToKeep]),2.5,0);
...
AtA = @(x) AtA_UV(FT,x,V,csm_lowRes,size(kdata_com,1)) + 0.3*Reg(x);
...
save(strcat('res_new',str,'.mat'), 'y','lowResRecons','-v7.3');
end
