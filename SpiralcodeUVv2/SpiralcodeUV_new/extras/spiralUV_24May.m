...
addpath(genpath('./../../Data/SpiralData_UVirginia'));
addpath('./csm');
...
save(strcat('data_',str,'.mat'),'csm','kdata','ktraj_scaled','dcf');
%% ===============================================================
...
% [~, tmp, L] = est_laplacian_LR(lowResRecons,FT_LR,kdata_com,csm_lowRes, 4.5, lam(jj));
% W=(circshift(eye(500),[0,1])+circshift(eye(500),[1,0]));
% W=(W+W')/2;
% Lnn = (diag(sum(W,1))-W);
% % [~,~,L]=estimateLapKernelLR(reshape(lowResRecons,[N1*N1,numFramesToKeep]),sigma(1),lam(jj));
% % [~,Sbasis,V]=svd(L+0.01*Lnn);
...
% % 
% % tic; x = solveUV(ktraj_scaled,kdata,csm, V, N, 20,lambdaSmoothness*Sbasis,useGPU);toc
% % y = reshape(reshape(x,[N*N,nBasis])*V',[N,N,numFramesToKeep]);
% %%
% %clear kdata csm V ;
% mkdir './../../Data/Results/res_May26';
% cd './../../Data/Results/res_May26';
% save(strcat('res_',str,'_',num2str(sigma),'_',num2str(lam(jj)),'.mat'), 'tmp','lowResRecons','L','-v7.3');
