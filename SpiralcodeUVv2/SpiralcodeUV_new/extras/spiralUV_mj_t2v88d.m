...
[kdata_com,ktraj_com] = binSpirals(kdata,ktraj_scaled,600,9);
...
W=(W+W')/2;
Lnn = (diag(sum(W,1))-W);
[~,tp,L]=estimateLapKernelLR(reshape(lowResRecons,[N1*N1,numFramesToKeep]),sigma(kk),lam(jj));
[~,Sbasis,V]=svd(L);
...
tic; x1 = pcg(AtA,Atb(:),1e-5,40);toc;
...
%%
%clear kdata csm V ;
mkdir './../../Data/Results/resold_28May';
cd './../../Data/Results/resold_28May';
save(strcat('res_',str,num2str(sigma2(ll)),'_',num2str(sigma(kk)),'_',num2str(lam(jj)),'.mat'), 'y','lowResRecons','-v7.3');
