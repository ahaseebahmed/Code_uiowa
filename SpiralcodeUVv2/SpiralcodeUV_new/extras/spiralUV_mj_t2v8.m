...
[kdata_com,ktraj_com] = binSpirals(kdata,ktraj_scaled,600,15);
...
[~,Si,L]=estimateLapKernelLR(reshape(lowResRecons,[N1*N1,numFramesToKeep]),sigma(1),lam(p));
[~,Sbasis,V]=svd(L);
...
AtA = @(x) AtA_UV(FT,x,V,csm_lowRes,size(kdata_com,1)) + 0.8*Reg(x);
...
%%
clear kdata csm V ;
mkdir './../../Data/Results/res_sigma_lam';
cd './../../Data/Results/res_sigma_lam';
save(strcat('res_11_',num2str(sigma),'_',num2str(lam(p)),'.mat'), 'y','lowResRecons','-v7.3');
