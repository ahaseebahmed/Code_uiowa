...
[kdata_com,ktraj_com] = binSpirals(kdata,ktraj_scaled,2496,5,1);
...
tic;[lowResRecons,ini] = l2Recont_v3(kdata_com,FT_LR,csm_lowRes,0.01,N1);toc
...
% for ll=1:size(lam2,2)
 tic; x = solveUV(ktraj_scaled,kdata,csm, V, N, 50,0.01*Sbasis,useGPU);toc
 y = reshape(reshape(x,[N*N,nBasis])*V',[N,N,numFramesToKeep]);
%%
%clear kdata csm V ;
mkdir './../../Data/Results/res_26May';
cd './../../Data/Results/res_26May';
save(strcat('res0_',str,num2str(lam2(ll)),'_',num2str(sigma(kk)),'_',num2str(lam(jj)),'.mat'),'y','lowResRecons','-v7.3');
