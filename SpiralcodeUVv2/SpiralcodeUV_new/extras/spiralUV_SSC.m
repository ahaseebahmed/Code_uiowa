...
[L]=ssc_algo(reshape(lowResRecons,[N1*N1,numFramesToKeep]),0.1,0.0001);
L1=abs(L)+abs(L)';
[~,Sbasis,V]=svd(L);
...
AtA = @(x) AtA_UV(FT,x,V,csm_lowRes,size(kdata_com,1)) + 1.3*Reg(x);
tic; x1 = pcg(AtA,Atb(:),1e-5,50);toc;
lowResRecons = reshape(reshape(x1,[N1*N1,nBasis])*V',[N1,N1,numFramesToKeep]);
[L]=ssc_algo(reshape(lowResRecons,[N1*N1,numFramesToKeep]),0.5,0.0001);
[~,Sbasis,V]=svd(L);
V=V(:,1:nBasis);
Sbasis=Sbasis(1:nBasis,1:nBasis);
...
%%
clear kdata csm V ;
mkdir './../../Data/Results/res_11Apr';
cd './../../Data/Results/res_11Apr';
save(strcat('300_res_14Apr_',str,'.mat'), 'y','lowResRecons','-v7.3');
