...
[~,Si,L]=estimateLapKernelLR(reshape(lowResRecons,[N1*N1,numFramesToKeep]),sigma(1),lam(1));
[~,Sbasis,V]=svd(L);
...
AtA = @(x) AtA_UV(FT,x,V,csm_lowRes,size(kdata_com,1)) + 0.5*Reg(x);
...
%%
clear kdata csm V ;
mkdir './../../Data/Results/res_25Apr';
cd './../../Data/Results/res_25Apr';
save(strcat('res_',str,'.mat'), 'y','lowResRecons','-v7.3');
