...
tic; x = solveUV(ktraj_scaled,kdata,csm, V, N, 50,lam2(ll)*Sbasis,useGPU);toc
y = reshape(reshape(x,[N*N,nBasis])*V',[N,N,numFramesToKeep]);
%%
%clear kdata csm V ;
mkdir './../../Data/Results/res_16May';
cd './../../Data/Results/res_16May';
save(strcat('res_',str,num2str(lam2(ll)),'_',num2str(sigma(kk)),'_',num2str(lam(jj)),'.mat'), 'y','lowResRecons','-v7.3');
