...
SHRINK_FACTOR = 1.3;
nBasis = 12;
lambdaSmoothness = 3;%0.025;%10;%0.025;
...
% % ============================================================== 
 ...
 dat=[3];
 for jj=1:size(dat,2)
 dr1=strcat('./../../Data/SpiralData_Virg/Phantom/3T_1leave_BH_012_IR_FA5_TBW5_6_TR',num2str(dat(jj)));
 dr2=pwd;
 cd(dr1);
 load('kspaceData.mat');
 cd(dr2);
...
csm=giveEspiritMaps(reshape(vcoilImages,[size(vcoilImages,1), size(vcoilImages,2), nChannelsToChoose]),0.002);
...
cd(dr1);
save(strcat('res3_',num2str(lambdaSmoothness),'_',num2str(sigma(ii)),'_',num2str(lam(jj)),'.mat'), 'y','csm','-v7.3');
cd(dr2);
