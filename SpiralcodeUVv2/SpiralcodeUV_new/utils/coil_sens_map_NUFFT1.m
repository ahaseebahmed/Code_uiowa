function [csm,coilimages] = coil_sens_map_NUFFT1(kdata,trajectory,N,useGPU)

%[nReadouts,nInterleaves,nFrames,nCh] = size(kdata); 
[nReadouts,nInterleaves,nCh] = size(kdata); nFrames=1;
...
if(useGPU)
    ...
    FT = gpuNUFFT(ktraj_gpu,ones(nReadouts*nInterleaves*nFrames,1),osf,wg,sw,[N,N],[],true);
else
...
for i=1:nCh
    temp = FT'*kdata(:,i);
    coilimages(:,:,i)= reshape(pcg(ATA,temp(:),1e-3,50),[N,N]);
end
