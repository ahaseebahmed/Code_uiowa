function [csm,coilimages] = coil_sens_map_NUFFT3(kdata,kx,ky,kz,N,useGPU)
...
if(useGPU)
    ...
    ktraj_gpu = [kx(:),ky(:),kz(:)]';
    %ktraj_gpu=ktraj_gpu(1:2,:);
    
    FT = gpuNUFFT(ktraj_gpu,ones(nReadouts*nInterleaves*nFrames,1),osf,wg,sw,[N,N,N],[],true);
else
...
ATA = @(x) reshape(FT'*(FT*reshape(x,[N,N,N])),[N*N*N,1]);

kdata = reshape(kdata,[nReadouts*nInterleaves*nFrames,nCh]);
for i=1:nCh
    temp = FT'*kdata(:,i);
    coilimages(:,:,:,i)= reshape(pcg(ATA,temp(:),1e-3,50),[N,N,N]);
end
