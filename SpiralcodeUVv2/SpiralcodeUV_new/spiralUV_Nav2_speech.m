...
% numFrames=floor((numberSpirals-spiralsToDelete)/ninterleavesPerFrame);
 kdata=kdata(:,spiralsToDelete+1:numFrames*ninterleavesPerFrame,cRatioI(1:nChannelsToChoose));
...
kdata = kdata(:,:,framesToDelete+1:numFramesToKeep+framesToDelete,:);
...
cd(dr1);
save(strcat('res25_',num2str(lambdaSmoothness),'_',num2str(sigma(ii)),'_',num2str(lam(jj)),'_',num2str(sl),'.mat'), 'y','-v7.3');
cd(dr2);
