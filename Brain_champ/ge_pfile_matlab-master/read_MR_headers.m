function [ header, read_hdr_status ]  = read_MR_headers( filename, headerID, datatype )
%-----------------------------------------------------------------------------
...
switch headerID
    ...
    case 'qimg'
        ...
        %%% CAC 171214
    case 'rdb'
        % Read only the rdb header
        %errordlg('quick read mode is not supported');
        for i = 1:length(header_list)
            if header_list(i)==1		% rdb header
                fseek( fid, offset, 'bof');
                header.rdb_hdr = read_rdb_hdr( fid, rdbm_rev );
                %printptxcvs(header.rdb_hdr);
                rdb_header_offset = offset;
        ...
        %%% CAC 171214
        %%% only headers changed so far to read p-file version 24.000 data
    case 'cac_hack'
        % Step through all the headers present in the file, reading them in order
        for i = 1:length(header_list)
            fseek( fid, offset, 'bof');
        ...
        % CAC 180718, call the orchestra-sdk
    case 'orchestra'
        startDir = pwd;
        [DirPath, FName, FExt] = fileparts(filename);
        FName = strcat(FName, FExt);
        cd(DirPath);
        pfileHandle = GERecon('Pfile.Load', FName);
        ...
        % offset calculated from the hdr_sizes.txt values
        for i = 1:length(header_list)
            offset = offset + header_lengths(header_list(i));
        end
        off_data = header.orchestra_header.RawHeader.off_data;
        header.total_length = offset;
        if offset ~= off_data
            fprintf('\n');  warning( 'Replacing rdb_hrd.off_data %d with value %d expected from hdr_sizes.txt.', off_data, offset);
            header.rdb_hdr.off_data = offset;
        end
...
%   7=tool, 8=prescan, 9=exam, 10=series, 11=image, 12=suite, 13=pixel.
...
if strcmp(datatype,'raw') == 1
    ...
    if (hdr_sizes_file)
        ...
        if hdr_szs(1) == -1 %sacolick, if is host but cant find proper pfile revision, revert to compiled hdr_sizes.txt
            hdr_szs = read_headerinfo(fullfile(fileparts(mfilename('fullpath')), 'hdr_sizes.txt'), rdbm_rev);
        end
        ...
        if hdr_szs(1) ~= -1
            ...
            if hdr_szs(1) ~= -1
                ...
                % sacolick, redflag if is host, and still can't find the
                % proper pfile revision, try compiled hdr_sizes.txt
                hdr_szs = read_headerinfo('hdr_sizes.txt',rdbm_rev);
                if hdr_szs(1) ~= -1
                    formatID = ['pfile',num2str(hdr_szs(1))];
                    endianID = 'ieee-be';
