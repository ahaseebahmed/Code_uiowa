...
if rdbm_rev == 20.006 
  ...
  a.excess = fread(my_file, 462, 'int16');
...
if rdbm_rev == 21.001 
  ...
  a.echo_pc_yxfitorder = fread(my_file, 1, 'uint16');
  a.padding = fread(my_file, 1, 'int16');
  a.ptx = fread(my_file, 1, 'int32');
  a.ptx_blochsiegert_constant = fread(my_file, 1, 'float32');
  a.ptx_blochsiegert_b1 = fread(my_file, 1, 'float32');
  a.ptx_b1map_txmode = fread(my_file, 1, 'int32');
  a.ptx_delta_te = fread(my_file, 1, 'int32');
  a.xmtaddaps1 = fread(my_file, 1, 'float32');
  a.tx_nchannels = fread(my_file, 1, 'int32');
 ...
 % RDBM revision 24.000 and 20.007 same rdb_hdr as 20.006
 % CAC 171214 from https://github.com/njvack/pfile-tools
 % also 21.001?
if (rdbm_rev == 20.007) |  (rdbm_rev == 24.000) 
  a.rdbm_rev = fread(my_file, 1, 'float32');
  a.run_int = fread(my_file, 1, 'int32');
  a.scan_seq = fread(my_file, 1, 'int16');
  a.run_char = freadc(my_file, 6);
 ...
 % RDBM revision 26.002 is likely to be error prone
 %  CAC 180718
if (rdbm_rev == 26.002)
  a.rdbm_rev = fread(my_file, 1, 'float32');
  a.off_data = fread(my_file, 1, 'int32');
