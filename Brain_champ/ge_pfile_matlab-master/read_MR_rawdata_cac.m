...
% DEBUG_FLAG = 0;   % quiet
DEBUG_FLAG = 4;    % methods debug displayed
...
% for reading Silence p-file, *** CAC 180222
nframes     = header.rdb_hdr.nframes;
...
% do and Arg check...missing? *** CAC 180223
phaselist = 1:nphases;
echolist  = 1:nechoes;
slicelist = 1:nslices;
% rcvrlist  = 1:receivers;
rcvrlist  = 1;      % for debugging, fast load of single channel, *** CAC 180223
save_baseline = 'db';
...
for filei = 1:npasses
    ...
    if fidi < 3
        ...
        if filesize~=total_bytes
            status_str = sprintf('Wrong rawfile size: %d bytes (actual) %d bytes (expected) \n', filesize, total_bytes);
            raw_read_status = -1;
            fprintf( '  Pass %2d rawfile:%12s   %s\n', filei, filename, status_str);
            return;
        else
            status_str = sprintf('OK  size: %d bytes  header %d bytes \n', filesize, header.total_length);
        end
    end
    if DEBUG_FLAG > 0
        fprintf( '  Pass %2d rawfile:%12s   %s\n', filei, filename, status_str);
