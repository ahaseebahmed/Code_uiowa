function  [x,Atb] = solveUV( ktraj, w, kdata, csm, V, N, nIterations, SBasis, useGPU)
...
if sTraj(1) == 3 % 3d
    ...
    if useGPU
        ...
        if mask ==1
            tmp=FT'*(kdata(:).*w(:).^1.2);
            gw1 = giveMask(abs(tmp),15,0);
        %csm=1-gw1;
            gw1=reshape(gw1,[N*N*N,1]);
            gw1=repmat(gw1,[1,nbasis-1]);
            gw2(:,1:nbasis-1)=gw1;
        end
        ...
        Atb = Atb_UV( FT, kdata.*w, V, csm, N, true,imageDim);
        Reg1 = @(x) reshape( reshape( x, [N*N*N, nbasis]) * SBasis, [N*N*N*nbasis, 1]);
        %Reg2 = @(x) reshape(reshape(x,[N*N*N,nbasis]).*gw2,[N*N*N*nbasis,1]);
        AtA = @(x) AtA_UV( FT, x, V, csm, N, nSamplesPerFrame,imageDim,w) + Reg1(x);%+0.1*Reg2(x);
    else
        FT = NUFFT( ktraj/N, 1, 0, 0, [N, N, N]);
        Atb = Atb_UV( FT, kdata, V, csm, false, imageDim);
        AtA = @(x) AtA_UV( FT, x, V, csm, nSamplesPerFrame, imageDim);
    end
    x = pcg_quiet( AtA, Atb(:), 1e-5, nIterations,[],[],Atb(:)); %size( x)
