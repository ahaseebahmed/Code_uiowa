...
% define operators
eigHtH      = abs(H).^2;%abs(fftn(H, [rows cols frames])).^2;
dx = zeros(1,2,class(H));dx = [1,-1];
dy = zeros(2,1,class(H));dy = [1;-1];
dz = zeros([1,1,2],class(H));dz(1,1,1)=1; dz(1,1,2)=-1;

eigDtD      = abs(beta(1)*fftn(dx,  [rows cols frames])).^2 + abs(beta(2)*fftn(dy, [rows cols frames])).^2;
if frames>1
    eigEtE  = abs(beta(3)*fftn(dz, [rows cols frames])).^2;
else
    eigEtE = 0;
end

Htg         = ifftshift(ifftn(conj(H).*fftn(fftshift(g))));%imfilter(g, H, 'circular');
[D,Dt]      = defDDt(beta);
...
for itr=1:max_itr
    % solve f-subproblem
    f_old = f;
    rhs   = fftn(fftshift((mu/rho)*Htg + Dt(u1-(1/rho)*y1,  u2-(1/rho)*y2, u3-(1/rho)*y3)));
    eigA  = (mu/rho)*eigHtH + eigDtD + eigEtE;
    f     = ifftshift((ifftn(rhs./eigA)));
    ...
    % update rho
    if (opts.print==true)
        r1         = ifftshift(ifftn(H.*fftn(fftshift(f))))-g;
        r1norm     = sum(abs(r1(:)).^2);
        r2norm     = sum(sqrt(abs(Df1(:)).^2 + abs(Df2(:)).^2 + abs(Df3(:)).^2));
        objval     = (mu/2)*r1norm+r2norm;
    end
    
    rnorm_old  = rnorm;
    rnorm      = sqrt(norm(abs(Df1(:)-u1(:)), 'fro')^2 + norm(abs(Df2(:)-u2(:)), 'fro')^2 + norm(abs(Df3(:)-u3(:)), 'fro')^2);
    ...
    % relative change
    relchg = norm(abs(f(:)-f_old(:)))/norm(abs(f_old(:)));
    out.relchg(itr) = gather(relchg);
    
    if (opts.print==true)
        out.objval(itr) = gather(objval);
    end
    
    % print
    if (opts.print==true)
        fprintf('%3g \t %6.4e \t %6.4e \t %6.4e \t %6.4e \t %6.4e\n ', itr, relchg, r1norm, r2norm, objval, rho);
        imagesc(abs(f(:,:,153))); drawnow
    end
