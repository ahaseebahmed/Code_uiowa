...
nSpokesToUse = 400*375;
dd = reshape(dd(:,1:nsamples*nSpokesToUse),[nvCh,nsamples,nSpokesToUse]);
k = reshape(k(1:nsamples*nSpokesToUse,:),[nsamples,nSpokesToUse,3]);
dcf = reshape(dcf(1:nsamples*nSpokesToUse),[nsamples,nSpokesToUse]);
...
k2= k*nudgefactor*mtx_acq/mtx_reco;
...
dcf2 = reshape(dcf(indices,:,:),nsamplesNew*nanglesNew,1);
dd_2 = reshape(dd(:,indices,:),nvCh,nsamplesNew*nanglesNew);
...
parfor i=1:nvCh,
   coilImages(:,:,:,i) = gridding3dsingle(k2,dd_2(i,:),dcf2,[1 1 1]*mtx_reco);
end
...
dd_1 = zeros(nCh,nsamples,nSpokesinIR,floor(nangles/nSpokesinIR),class(dd));
...
for nIR=1:400,%floor(nangles/nSpokesinIR)-2 % more noise in later images; motion ??
    nIR
    pause(0.1)
    dd_1(:,:,:,nIR) = reshape(dd(:,:,startSpoke:startSpoke+nSpokesinIR-1),[nCh,nsamples,nSpokesinIR,1]);
