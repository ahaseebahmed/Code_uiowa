...
nSpokesToUse = size(dd,3);
dd1 = reshape(dd,[nCh,nsamples,nSpokesToUse]);
k1 = k;%reshape(k(1:nsamples*nSpokesToUse,:),[nsamples,nSpokesToUse,3]);
dcf1 = reshape(dcf,[nsamples,nSpokesToUse]);
dcf1(:,badindices)=0;
...
dcftemp = reshape(dcf1(indices,:),[1,nsamplesNew*nangles]);
...
[cmap,mask] = giveCSM(coilImages,5,12,0.03);%
...
T1 = [0.01,0.001,0.1,14,17,30,45,60,75,90,110,120],
T1 = log(2)./T1;

%%
Nbasis = 3;
%t = 1:sz(3);t = t(:);
t = ones(10,1)*[1:35]*10;t = t(:);
...
coeffs = pcg(Q,Atb(:),1e-8,80,[],[],Atb(:));
coeffs = reshape(coeffs,mtx_reco^3,size(Vbasis,2));
%%
N=30;
for i=1:10:120%size(Vbasis,1)
    test = reshape(coeffs*Vbasis(i:i+N-1,:)',[mtx_reco*[1,1,1],N]);
    test = mean(abs(test),4);
    figure(1);imagesc(imrotate(squeeze(abs(test(:,floor(0.4*mtx_reco),:))),-90),[0,0.2*max(abs(test(:)))]); 
    figure(2);imagesc(imrotate(squeeze(abs(test(:,:,floor(0.5*mtx_reco)))),-90),[0,0.2*max(abs(test(:)))]); 
    title(num2str(i));
    pause(0.1);
end
%%
