...
directory1='/Users/ahhmed/pytorch_sense/'
...
with h5.File(directory+'ktraj_rad.h5','r') as f: 
  ktraj=np.asarray(f['/ktraj/re'])
  ktraj=ktraj+np.asarray(f['/ktraj/im'])*1j
  ktraj=ktraj.astype(np.complex64)/nx
  ktraj=np.squeeze(np.transpose(ktraj[0:NF,:,:],[0,1,2]))*2*np.pi
ktraj=np.reshape(ktraj,(1,NF*nintl*nkpts))
ktraj1=np.reshape(ktraj,(NF,nintl*nkpts))  
...
with h5.File(directory+'L.h5','r') as f:
  ...
  V=V[NF-nbasis:NF,:]
  sbasis=np.diag(sbasis[NF-nbasis:NF,])
...
smap1=np.tile(smap,(nbasis,1,1,1,1))
smapT = torch.tensor(smap1).to(dtype)

smap2=np.tile(smap,(NF,1,1,1,1))
smapT2 = torch.tensor(smap2).to(dtype)
...
ktraj1 = np.stack((np.real(ktraj1), np.imag(ktraj1)), axis=1)
#ktraj1=np.tile(ktraj1,(nbasis,1,1))
ktrajT1 = torch.tensor(ktraj1).to(dtype)

atb = np.stack((np.real(atb), np.imag(atb)), axis=1)
atbT = torch.tensor(atb).to(dtype).unsqueeze(1)
...
#xx=xx.to(gpu)
#%% run cg-sense
B=lambda x: AtA(x)+lam*sf.basis_multi(x,sbasis)
sf.tic()
recT=sf.pt_cg(B,atb,cgIter,cgTol)
sf.toc()
