...
#                yy=torch.reshape(kdataT.cuda(),(NF,3,2,nintl,nx))
#                yy[:,:,:,0,:]=0
#                yy[:,:,:,1,:]=0
#                yy=torch.reshape(yy,(NF,3*2*nintl*nx))
#                for i in range(nbasis):
#                    tmp=torch.diag(v1[i])@yy
#                    tmp=torch.reshape(tmp,(NF,nch,2,nintl*nx))
#                    tmp=tmp.permute(1,2,0,3)
#                    tmp=torch.reshape(tmp,(1,nch,2,NF*nintl*nx))
#                    yy1[i]=adjnufft_ob(tmp*dcf[i].unsqueeze(0),ktrajT[i].unsqueeze(0).cuda(),smap=smapT[i].unsqueeze(0).cuda())
    ...
    #                l1_reg += param.abs().sum()
                ...
                err=abs(kdataT.cuda()-yy).pow(2)
                loss=(df2.cuda()*err).sum()#+0.1*(z.cpu()-u2.cpu()).pow(2).sum()+0.1*(z1[0,0].cpu()-v2.cpu()).pow(2).sum()
                #loss=abs(u1-atb).pow(2).sum()
                #del yy
                #loss=loss.cuda()
                #if ep1%5==0:
