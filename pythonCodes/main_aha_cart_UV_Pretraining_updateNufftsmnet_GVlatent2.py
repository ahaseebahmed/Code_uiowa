...
sf.tic()
recT1=sf.pt_cg(B,atb1,recT1,18,1e-10)
sf.toc()
recT1=recT1/mx
...
G.load_state_dict(torch.load('tempp_8000.pt'))
...
G.load_state_dict(torch.load('temppU1.pt'))        
...
#          
    ...
    loss=((dcf[0,:,:,0:nf1].pow(1))*(err)).sum()+0.00001*l1_regU+0.0001*l1_regV+0.0*Smoothness(z1[0,0,:,:]) 
...
rec1=rec.T@V[:,20:27]
rec1=np.reshape(rec1,(nx,nx,7))
#sio.savemat('stmU_900.mat',{"rec":rec1}) 
