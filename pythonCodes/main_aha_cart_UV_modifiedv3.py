...
sf.tic()
ep_count=200   
for bas in range(28,-1,-1):
    ep_count=int(ep_count*0.8)
    if bas<=19:
        ep_count=100
    for ep2 in range(ep_count):
        indx=torch.randint(0,NF,(300,),device='cuda')
        B1=B[:,indx].cuda()
        maskT1=maskT[indx,].cuda()
        B1=torch.reshape(B1,(nch,int(NF1/batch_sz),batch_sz,nx*nx*2))
        maskT1=torch.reshape(maskT1,(int(NF1/batch_sz),batch_sz,nbasis,nx*nx))
        for bat in range(int(NF1/batch_sz)):
            mask_b=maskT1[bat]
            ...
            b_est=F(u1,maskb)
            loss=0.5*(b_est-B1[:,bat]).pow(2).sum()#+(sT@W*U).pow(2).sum()
            if ep2%5==0:
                print(ep2,loss.item())
...
#sio.savemat('temp33_pro_gen900.mat',{"rec":rec1}) 
