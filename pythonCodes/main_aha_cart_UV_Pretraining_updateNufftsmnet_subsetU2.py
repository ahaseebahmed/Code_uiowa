...
at1=np.zeros((sz,nbasis*2,nx,nx))
at=np.zeros((sz,nbasis*2,nx,nx))
v0=np.zeros((sz,nbasis,NF))
v=np.zeros((sz,nbasis,nf1))

for ep1 in range(10):
    #rndm=random.sample(range(sz),sz)
    for fl in range(4,sz):
        #%%
        #fl=26
...
#        X=np.reshape(X,(nbasis,1,1,nf1,1))
        v1=np.reshape(X[:,0:nf1],(nbasis,1,1,nf1,1))
...
#            z2=VT[:,itt*nf1:nf1*(itt+1)].unsqueeze(0).unsqueeze(0).cuda()
    ...
    #mx=torch.max(atb)
        ...
        xx=torch.tensor(v1).permute(0,3,1,2,4)
        #xx=v1.permute(0,3,1,2,4)
        torch.cuda.empty_cache()
        
        a1=torch.tensor(d2['at1']).to(dtype)
        a2=torch.tensor(d2['at']).to(dtype)
        v1=torch.tensor(d2['v']).to(dtype)
        v2=torch.tensor(d2['v0']).to(dtype)
        v2=v2[:,:,0:nf1]
        
        a0=a1[4:27]
        z1=torch.FloatTensor(3,23,2*nbasis,nx,nx).fill_(0) 
        noi=[0.05,0.01,0.001]
        for it in range(3):
            z1[it]=a0+torch.normal(0,noi[it],(a0.shape[0],a0.shape[1],a0.shape[2],a0.shape[3]))
        z=a0.repeat(3,1,1,1,1)
        
        z1=torch.reshape(z1,(3*23,nbasis*2,nx,nx))
        z=torch.reshape(z,(3*23,nbasis*2,nx,nx))
        
        sf.tic()
        for ep in range(100):
            ls=0
            #rndm=random.sample(range(23*3),23*3)
            for rn in range(69):
                z0=z1[rndm[rn]].unsqueeze(0)
                ...
                atb=torch.reshape(z[rndm[rn]],(nbasis,2,nx,nx))
                l1_reg = 0.
                for param in G.parameters():
                    l1_reg += param.abs().sum()
                #loss=abs((u1.unsqueeze(1)*v1[rn].cuda().unsqueeze(2).unsqueeze(3).unsqueeze(4)).sum(dim=0)-(atb.cuda().unsqueeze(1)*v2[rn].cuda().unsqueeze(2).unsqueeze(3).unsqueeze(4)).sum(dim=0)).pow(2).sum()
                loss=(abs(u1-atb.cuda())).pow(2).sum()+0.01*l1_reg
            ...
            print(ep,loss.item())
            if ep%100==0:
                torch.save(G.state_dict(),'tempp_'+str(ep)+'.pt')
        sf.toc()
...
#        #--------------------------------------------------------------------------------
        ...
        z2=torch.reshape(torch.tensor(np.squeeze(X[:,0:nf1])),(1,1,nbasis,nf1))
        #GV=SmallModel().to(gpu)
        #G.load_state_dict(torch.load('tempp.pt'))
        optimizer=torch.optim.AdamW([{'params':G.parameters(),'lr':1e-4}])
        scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.7, patience=6, verbose=True, min_lr=1e-5)
#        
#        xx=torch.reshape(atb1,(nbasis,2,nx,nx))
#        plt.figure(2)
#        dd=(np.reshape(X[:,:,:,26],(nbasis,1,1,1))*xx.detach().cpu().numpy()).sum(axis=0)
...
#            for bat in range(1):
            ...
            v1=torch.reshape(v1[0],(1,1,nbasis,nf1,1))
            u1=(atb1.cuda())#+z.cuda()
            u1=torch.reshape(u1,(nbasis,2,nx,nx))
            u1=u1.unsqueeze(1)
...
rec0 = np.squeeze((z0).detach().cpu().numpy())
rec0=np.reshape(rec0,(nbasis,2,nx,nx))
rec0 = rec0[:,0] + 1j*rec0[:,1]

rec00=np.concatenate((rec,rec0),axis=-1)

       
plt.figure(3)
for i in range(30):
    plt.imshow((np.squeeze(np.abs(rec00[i,100:400,:]))),cmap='gray')       
    plt.show()
    #plt.colorbar()
    plt.pause(1)
##    
...
rec=np.reshape(rec,(nbasis,nx*nx))
rec1=rec.T@np.squeeze(Vx[rn,:,0:nf1])
