...
sf.tic()
recT=sf.tor_conjgrad(B,atb,recT,4,1e-5)
sf.toc()
...
B=lambda x: ATUV1(AUV1(x,v2))#+Sbasis2(sb,x)
sf.tic()
recT1=sf.tor_conjgrad(B,atb1,recT1,8,1e-10)
sf.toc()
recT1=recT1/mx
...
optimizer=torch.optim.Adam([{'params':GV.parameters(),'lr':1e-3},{'params':z1,'lr':0e-3}])
...
for ep1 in range(1000):#25000
    for bat in range(1):
        ...
        v1=GV(z1)
        #v1=v1.permute(1,0)
        loss=abs(v1[0,0]-V1).pow(2).sum()#+0.000*l1_reg+1000.0*KLloss(z1[0,0])#+1.0*Smoothness(z1[0,0,:,:]) 
        if ep1%10==0:
            print(ep1,loss.item())
...
for ep1 in range(1000):#25000
    for bat in range(1):
        ...
        pp=np.append(pp,psnr)
#%%
#z=torch.normal(0,0.03,(atb1.shape[0],atb1.shape[1],atb1.shape[2],atb1.shape[3],atb1.shape[4])).cuda()#.permute(1,0,2,3)
#z=torch.reshape(z,(1,2*nbasis,nx,nx))
...
GV.load_state_dict(torch.load('temppV_0.pt')) 
G.load_state_dict(torch.load('temppU_0.pt')) 

z1=torch.load('temppZ_0.pt')      
z=torch.load('temppZ0_0.pt')   
...
#          
    ...
    loss=((dcf[0,:,:,0:nf1].pow(1))*(err)).sum()+0.0000001*l1_regU#+0.0000*l1_regV+0.3*Smoothness(z1[0,0,:,:])+1.0*KLloss(z1[0,0])
...
#            loss=(err).sum()#+10*l1_reg#+0.1*(z.cpu()-u2.cpu()).pow(2).sum()+0.1*(z1[0,0].cpu()-v2.cpu()).pow(2).sum()
    if it%100==0:
        ...
        org2=(v2[0,0,:,20].unsqueeze(1).unsqueeze(1)*recT.squeeze(1)).sum(dim=0).detach().cpu().numpy()
...
#sio.savemat('blurfinal_lam1_1.mat',{"rec":rec1}) 
