...
#%%
#def AUV(u1,v1,nufft_ob,ktrajT):
#    yy=torch.cuda.FloatTensor(nch,2,NF,nx*nintl).fill_(0)
#    for i in range(nbasis):
#        xx=nufft_ob(u1[i].unsqueeze(0),ktrajT[i].unsqueeze(0).cuda())
#        xx=torch.reshape(xx,(nch,2,NF,nx*nintl))
#        yy=yy+v1[:,:,i]*xx
#        
#    return yy
#
#def ATUV(yy,v1,dcf,adjnufft_ob,ktrajT):
#    yy1=torch.cuda.FloatTensor(nbasis,1,2,nx,nx).fill_(0) 
#    for i in range(nbasis):                    
#        tmp=v1[:,:,i]*yy
#        tmp=torch.reshape(tmp,(1,nch,2,NF*nintl*nx))
#        yy1[i]=adjnufft_ob(tmp*500*(dcf[i]).unsqueeze(0).cuda(),ktrajT[i].unsqueeze(0).cuda())
#        
#    return yy1
...
#            for bat in range(1):
            ...
            loss=((dcf[0,:,:,0:nf1])*(err)).sum()#+0.000005*l1_reg#+Smoothness(v1[0,0,:,:,0]) 
...
#                torch.save(u1,'tmpU'+str(it)+'.pt')
            ...
            dd=(v1[0,0,:,10:50].unsqueeze(-1).unsqueeze(-1).detach().cpu()*u1[:,:,:,100:400,100:400].detach().cpu()).sum(dim=0).detach().cpu()
            org1=(v2[0,0,:,10:50].unsqueeze(-1).unsqueeze(-1).detach().cpu()*recT[:,:,:,100:400,100:400].detach().cpu()).sum(dim=0)
            psnr=torch.mean(myPSNR(org1,dd))
...
#sio.savemat('tmp11_300.mat',{"rec":rec1}) 
