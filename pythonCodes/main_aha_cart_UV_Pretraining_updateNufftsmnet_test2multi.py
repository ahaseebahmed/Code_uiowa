...
#            z2=VT[:,itt*nf1:nf1*(itt+1)].unsqueeze(0).unsqueeze(0).cuda()
        ...
        B=lambda x: ATUV1(AUV1(x))
        sf.tic()
        recT1=sf.pt_cg(B,atb1,recT1,16,1e-5)
        sf.toc()
        recT1=recT1/mx
        
        recT1=torch.reshape(recT1,(1,nbasis*2,nx,nx))
        recT=torch.reshape(recT,(1,nbasis*2,nx,nx))
        at1[fl]=recT1.cpu().numpy()
        at[fl]=recT.cpu().numpy()
        v0[fl]=V
        
        
        
        #atb1=atb1.squeeze(1)
        d2=sio.loadmat('/Shared/lss_jcb/abdul/prashant_cardiac_data/Data/meas_MID00185_FID142454_SAX_gNAV_gre3_datall.mat')
        a1=torch.tensor(d2['at1']).to(dtype)
        a2=torch.tensor(d2['at']).to(dtype)
        indx=np.array([14,15,16,18,19,21,22,23,25,26])
        a1=a1[indx]
        a2=a2[indx]
        #a1=a1[14:27]
        #a2=a2[14:27]
        ...
        G.load_state_dict(torch.load('tempp_24000.pt'))
        ...
        z=a1.to(gpu)
        #z1=
        #z=z.to(dtype)#+torch.normal(0,0.02,(recT1.shape[0],recT1.shape[1],recT1.shape[2],recT1.shape[3],recT1.shape[4])).cuda()
...
#        z=z.repeat(3,1,1,1)
        ...
        for rn in range(10000):
            rndm=random.sample(range(10),10)
            z1=z[rndm]
            ...
            loss=((abs(u1-a2[rndm].cuda()))).pow(2).sum()
            if rn%10==0:
                print(rn,loss.item())
        ...
        sf.toc() 
        torch.save(G.state_dict(),'tempp_'+str(rn+1)+'.pt')
...
#            for bat in range(1):
            ...
            err=abs(AUV(u1)-kdataT1.cuda()).pow(2)
            loss=(dcf[0]*(err)).sum()
...
#    
# 
u2=torch.reshape(u1[9],(nbasis,2,nx,nx))
...
rec = np.squeeze(a2[[26]].detach().cpu().numpy())
...
rec1=rec.T@V[[25],:,0:nf1]
rec1=np.reshape(rec1,(nx,nx,nf1))
#sio.savemat('tmp11_900.mat',{"rec":rec1}) 
