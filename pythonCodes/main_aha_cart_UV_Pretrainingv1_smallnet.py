...
G.load_state_dict(torch.load('wts-Uuu96.pt'))
GV.load_state_dict(torch.load('wts-Vvv96.pt'))
...
#    sT=sT.to(gpu)
    ...
    z=ss.ATBV(kdataT.to(gpu),csmT,VT)
    z=z.permute(3,0,1,2)
    ...
    for ep1 in range(30):
        ...
        for bat in range(bb_sz):
            ...
            #mask_b=maskT[bat]
            m_res=ss.maskV_cpu(maskT[bat],v1[bat])
    ...
    sf.toc()
    if fl%10==0:
        wtsFname1='wts-smallU'+str(fl+1)+'.pt'
        torch.save(G.state_dict(),wtsFname1) 
        wtsFname2='wts-smallV'+str(fl+1)+'.pt'
        torch.save(GV.state_dict(),wtsFname2)      
#%%
#Vz=v1.detach().cpu().numpy()
#
