...
writer.add_graph(unet,torch.randn(1,2,270,270).to(gpu))
trnFiles=os.listdir('./../Codes/TensorFlow_SToRM/Data/')
for ep in range(epochs):
    epLoss = 0.0
    epStart=time.time()
    for fl in range(len(trnFiles)):
        ...
        for i, (org,inp) in enumerate(loader):
