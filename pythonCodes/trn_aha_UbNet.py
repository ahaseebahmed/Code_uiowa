...
sz=len(trnFiles)
data2=np.zeros((sz,2*nbasis,N,N))
...
#    data=np.matmul(data1,data0)
    ...
    data1=np.reshape(data1,(nbasis,N,N))
    #data=data[0:6*n_select,]
    data1=np.fft.fftshift(np.fft.fftshift(data1,1),2) 
#        data=data[:,110:380,110:380]
    data1=data1.astype(np.complex64)
    data1=np.stack((np.real(data1),np.imag(data1)),axis=0)
    data1=np.reshape(data1,(2*nbasis,N,N))

    #data=(data)/(np.max(np.abs(data)))
    #data1=np.reshape(data1,(1,1,nbasis,N,N))
    data2[fl]=data1
