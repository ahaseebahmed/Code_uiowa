...
dtype = torch.float
directory1='/Users/ahhmed/pytorch_unet/SPTmodels/'
chkPoint='41'
...
def rhs(u1,D):
    ...
    x=x.permute(0,3,1,2)
    trnDs=rd.DataGen(x.detach().cpu().numpy(),x.detach().cpu().numpy())
    ldr=DataLoader(trnDs,batch_size=1,shuffle=True,num_workers=8,pin_memory=True)
    for i, (data,org) in enumerate(ldr):
...
for ep in range(epochs):    
    ...
    optimizer.step()
    epLoss+=loss.item()
    recT.requires_grad=False
    del atbT1
    #sf.toc()
    if ((ep+1) % 50==0) or ((ep+1)==epochs) :
        wtsFname='wts-'+str(ep+1)+'.pt'
