...
#        df1=torch.reshape(df1,(900,3*10240))
        ...
        for it in range(400):    
            for bat in range(1):
                ...
                #z=atb+torch.normal(0,noi[bat],(atb.shape[0],atb.shape[1],atb.shape[2],atb.shape[3])).cuda()
                #z=torch.reshape(z,(1,2*nbasis,nx,nx))
                u1=G(z.cuda())#+z.cuda()
                #u2=u1
                u1=torch.reshape(u1,(nbasis,2,nx,nx))
    ...
    #                l1_reg += param.abs().sum()
                ...
                err=abs(atb-yy1.squeeze(1)).pow(2)
                loss=(err).sum()#+0.1*(z.cpu()-u2.cpu()).pow(2).sum()+0.1*(z1[0,0].cpu()-v2.cpu()).pow(2).sum()
                print(it,loss.item())
#                plt.figure()
#                plt.plot(kdataT[1,1:10000].detach().cpu().numpy())
#                plt.plot(yy[1,1:10000].detach().cpu().numpy())
#                plt.show()
#                plt.pause(0.01)
...
#sio.savemat('tmp11.mat',{"rec":rec1}) 
