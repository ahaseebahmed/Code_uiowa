...
del recT
    #%% class for genenrator based storm method   
class AUV(nn.Module):
...
for i in range(1):
    ...
    for ep2 in range(ep_count):
        ...
        for bat in range(bb_sz):
            ...
            if bas[i]!=nbasis-1:                
                for t1 in range(nbasis-2,bas[i]-1,-1): 
                #maskb=mask_b[:,bas].unsqueeze(1)
                    maskb=torch.cat((mask_b[:,t1].unsqueeze(1),maskb),dim=1)
            del mask_b
            maskb=maskb.unsqueeze(3).repeat(1,1,1,2)
            maskb=torch.reshape(maskb,(batch_sz,nbasis-bas[i],nx*nx*2))
            U=G(z)
            ##U=U.permute(1,2,3,0)
            ...
            if bas[i]!=nbasis-1:                
                for t2 in range(nbasis-2,bas[i]-1,-1):                
                    u1=torch.cat((U[t2].unsqueeze(0),u1),dim=0)
                #u1=U[bas].unsqueeze(0)
...
#                print(ep2,loss.item())
            ...
            optimizer.step()
            scheduler.step(loss.detach())
        loss_U=(u1[29]-recT[29]).pow(2).sum()
...
for ep1 in range(31):
    for bat in range(bb_sz):
        ...
        u1=torch.reshape(u1,(nbasis,nx*nx*2))
        b_est=F(u1,mask_b)
        loss=0.5*(b_est-B[:,bat].cuda()).pow(2).sum()#+(sT@W*u1).pow(2).sum()
        if ep1%5==0:
            print(ep1,loss.item())
