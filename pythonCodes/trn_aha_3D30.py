...
patch_size=32
strde=32
noise_level=[0.05,0.3,0.01,0]#[0.2,0.09,0.01]#[0.1,0.07,0.03]
n_select=30
...
unet=SmallModel()
#modelDir1='SPTmodels/21Jun_110930pm_100ep_21Jun/wts-'+str(100)+'.pt'
...
sz=len(trnFiles)
data2=np.zeros((sz,3,n_select,N,N)).astype(np.complex64)
...
for fl in range(sz):
    ...
    data1 = matfile['U1']
    data0=data0[:,0:3*n_select]
    data=np.matmul(data1,data0)
    data=np.transpose(data,(1,0))
    data=np.reshape(data,(3*n_select,N,N))
    data=data[0:3*n_select,]
...
data2=np.reshape(data2,(sz*3,n_select,N,N))
inp=np.stack((np.real(data2),np.imag(data2)),axis=1)

Inp=torch.tensor(inp)
trnOrg=torch.tensor(inp)
