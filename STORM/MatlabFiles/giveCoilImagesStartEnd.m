...
function coilimages = giveCoilImagesStartEnd(kSpaceData,param,startframe,endframe)
...
ar =[1:param.lines_per_SG_block:endframe*param.lines_per_SG_block];
...
[X, Y] = giveRadialTrajectory(n,param.lines_per_SG_block,param.common_lines,param.lines_to_skip,endframe);

X = X(:,param.common_lines+1:end,startframe:endframe);
Y = Y(:,param.common_lines+1:end,startframe:endframe);
knew = knew(:,param.common_lines+1:end,startframe:endframe,:);
