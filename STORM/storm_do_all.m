function [U1,D,csm,vCoilImages,param,L] = storm_do_all(param,kSpaceData)
%function [b, bCom_smoothed,csm,S] = storm_do_all(param,kSpaceData)
...
nvchannels = 3;%size(vKSpaceData,2);
vCoilImages=vCoilImages(:,:,1:nvchannels);
vKSpaceData=vKSpaceData(:,1:nvchannels,:);
...
%param.gwin = gwin;
gwin = repmat(1+100*gwin,[1,1,param.Nbasis]);
...
bCom_img=bCom_img./max(abs(bCom_img(:)));
%%
% q = fft(bCom_img,[],2);
% q(:,param.nf/2-param.nf/4-10:param.nf/2+param.nf/4+10)=0;
% bCom_smoothed = ifft(q,[],2);

bCom_smoothed = bCom_img;

[W, bCom_est, L] = estimateLapKernelLR(bCom_smoothed, 0.25, 0.1,0);
% DD=(sum(W));
% aa=DD.^(-1/2);
% LL=diag(aa)*W*diag(aa);
% L=eye(param.nf)-LL;
[~,SBasis,D] = svd(L);
...
%% Solving for the coefficients
    ...
    %c1 = Ahb(b,S,conj(csm),param.n,param.nf,nvchannels,param.useGPU);
    %load('W1.mat');
    LamMtx = SBasis*param.lambdaCoeffs;
    gradU=@(z)(AhAUVVh_reg(z,D',S,csm,param.n,size(D,2),param.nf,nvchannels,LamMtx,[],[],param.useGPU));
    %gradU = @(z)(AhAUVVh_reg(z,D',S,csm,param.n,size(D,2),param.nf,nvchannels,LamMtx));
    %gradU = @(z)(AhA_reg(z,S,L,csm,param.n,param.nf,nvchannels,LamMtx,[],[],param.useGPU,gwin(:,:,1),W1,W2,b1,b2));
    tic;[U,~,~,~,err] = pcg(gradU,c1(:),10^-10,param.CoeffsCGIterations,[],[],c1(:));toc;
   ...
   if(param.SolverType=='L1')
    ...
    U1 = gather(reshape(c1,[param.n,param.n,nf]));    
