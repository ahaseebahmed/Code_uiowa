...
%%---------Reading data----------------
load(strcat('meas_MID00185_FID142454_SAX_gNAV_gre_L2_lam_2','.mat'));
y = reshape(reshape(U1,[N*N,nBasis])*D',[N,N,nframes]);
y = abs(fftshift(fftshift(y,1),2));
...
liste=dir(pwd);
for ij=1:length(liste)-2 

load(liste(ij+2).name);  
%y=abs(y);
...
% y=abs(y(130:210,130:210,:));
    ...
    y=y(N/2-N/4+1:N/2+N/4,N/2-N/4+1:N/2+N/4,pt1(1,1):pt2(1,1));

% y=imgaussfilt(y);
% kernel = -1*ones(3)./9;
% kernel(2,2) = 8/3;
% y = round(imfilter(y, kernel));
    cyc_exp=size(y1_exp,3);
    cyc_inp=size(y1_inp,3);


for ii=1:size(y,3)-cyc_exp-20
    tmp=y(:,:,ii:ii+cyc_exp-1);
    si1(ii)=norm(abs(tmp(:)-y1_exp(:)));
    %mi1(ij,ii)=MI_GG(abs(cc1(:)),abs(tmp(:)));    
end

for jj=1:size(y,3)-cyc_inp-20
    tmp1=y(:,:,jj:jj+cyc_inp-1);
    si2(jj)=norm(abs(tmp1(:)-y1_inp(:)));
...
for mm=1:cyc_exp
     mn1(mm)=norm(abs(y(:,:,in1)-y(:,:,in1+mm+10)));
     %mn2(ij,mm)=norm(abs(y(:,:,in2(ij))-y(:,:,in2(ij)+mm)));
end

for mm=1:cyc_inp
     %mn1(ij,mm)=norm(abs(y(:,:,in1(ij))-y(:,:,in1(ij)+mm)));
     mn2(mm)=norm(abs(y(:,:,in2)-y(:,:,in2+mm+10)));
end

    [~,in3]=min(squeeze(mn1));
    [~,in4]=min(squeeze(mn2));

    c_cyc_inp{ij}=y(:,:,in1:in1+in3+10);
    c_cyc_exp{ij}=y(:,:,in2:in2+in4+10);
clear y mn1 mn2;
clf;
end

save('FID142454.mat','c_cyc_exp','c_cyc_inp');
% 
% for i=1:no_slice
...
% end
% for kk=1:size(c_cyc_exp,2)    
% tmp=c_cyc_exp{kk};
% for i=1:size(tmp,3);imagesc(squeeze(abs(tmp(:,:,i)))); pause(); colormap gray;end
% end
