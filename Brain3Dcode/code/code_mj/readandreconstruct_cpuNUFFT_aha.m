...
load('/Shared/lss_jcb/abdul/data_0.25.mat');
...
str=1;%nangles=150000;
k1_new = reshape(k1(str:nS+str-1,1:nangles,:),nS*nangles,3);
dd_new = reshape(dd_v(:,str:nS+str-1,1:nangles),[nvCh,nS*nangles]);
dcf_new = reshape(dcf(str:nS+str-1,1:nangles),nS*nangles,1);
ind_new = reshape(ind(str:nS+str-1,1:nangles),nS*nangles,1);
...
FT = gpuNUFFT((k1_new)',ones(prod(sz(1)),1),osf,wg,sw,[1 1 1]*mtx_reco,[],true);
dtemp = reshape(dd_new,[nvCh,prod(sz(1))]);
dcftemp = dcf_new(:); 
...
% [cmap,mask] = giveCSM(coilImages,4,12);
...
%     FT = gpuNUFFT(k1_new',ones(size(k1_new,1),1),osf,wg,sw,[1 1 1]*mtx_reco,[],true);
%     tmp1=bsxfun(@times,dd_new',dcf_new);
%     Atb(:,:,:,:,i) = FT'*tmp1;
...
%     kspaceWts(:,:,:,i) = giveNUFFTx2(dcf_new(:,i),FT,mtx_reco,squeeze(k1_new)');
...
T = FourierWtOpSeries1(FT,cmap,true);

G = GradOp([mtx_reco,mtx_reco,mtx_reco],[1,1,1]);

M = @(x) T*x+5e-13*(G*x);

%Atb1 = sum(bsxfun(@times,Atb,conj(cmap)),4);clear Atb;

clear dcf dcf_new dcftemp coilImages dd dd_new dd_v dtemp ind ind_new k k1 k1_new nangles tmp tmp1
tic;recon = pcg(M,(Atb1(:)),1e-14,1,[],[],(Atb1(:)));toc
